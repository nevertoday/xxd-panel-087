<p align="center"><img src="./assets/banner.svg" alt="XXD Panel 087 项目横幅" width="1200"></p>
...
`sample-01`–`sample-04` 保留给未来可核验的原始发布样张。以上八张包括四张 16:9 横版左右样张和四张 3:4 竖版上下样张，均由 Panel 087 依据自己的原始提示词独立生成，不借用其他编号作品。样张使用英文智能配字。
...
```text
分析主体、轮廓、结构、姿态与叙事关系 → 拆解为少量有意义的 Nodes → 以真实线绳建立 Edges / Links → 围绕核心关系形成放射、聚合、三角或跨区连接 → 中心密、外围疏 → 加入图钉、纸签、针脚阴影、遮挡与手工误差 → 让手写注释进入节点附近和留白
```

## 成品中最容易识别的风格特点

- 每个节点都对应照片中的轮廓、结构转折、动作方向、位置或环境线索，不添加无意义节点。
- 线绳使用克制而鲜明的朱红、珊瑚红或暖红；背景保持自然白、暖白、浅灰或协调的极浅色。
- 节点可形成放射、聚合、三角、多边形和跨区域连接，但整体不能随机缠绕或铺满版面。
- 图钉、纸质标签、针脚阴影、线绳松紧差异与交叉遮挡共同制造真实装置感。
- 文字只作为少量手写观察注释，拒绝赛博 UI、犯罪板、卡通图钉、普通流程图和模板化思维导图。

原始审美约束与拒绝项只存在于[原始提示词](references/original-prompt/zh-CN.md)；Skill 与运行适配器只处理本次交付变量。 [Skill 工作流](SKILL.md) · [英文运行适配器](references/xxd-panel-087-prompt.en.md)
...
| **士兵级** | `xxd-panel-NNN`（当前 001–087） | 每个编号只执行自己独立的原始提示词与审美，把将军派发的单个任务完成为成品。 |
