...
`sample-01`–`sample-04`는 검증 가능한 원본 공개 예시를 위해 남겨 둡니다. 위 여덟 작품은 16:9 가로형 좌우 구성 네 장과 3:4 세로형 상하 구성 네 장입니다. 모두 Panel 087이 자체 원본 프롬프트에 따라 독립적으로 생성했으며 다른 번호의 작품을 재사용하지 않았습니다. 예시 문구는 영어로 자동 생성했습니다.

<!-- xxd-human-intro:start -->
## 이 스타일이 해결하는 문제

사진을 평범한 흐름도로 바꾸면 피사체 사이의 실제 공간 관계가 사라지고, 무작위 붉은 선과 화면을 가득 채운 노드는 값싼 수사 보드나 디지털 네트워크 UI처럼 보이기 쉽습니다.

**Panel 087**은 사진에서 피사체, 윤곽, 구조, 자세와 서사적 관계를 추출해 의미 있는 소수의 압정형 노드로 분해하고, 실제 실끈으로 다시 연결해 만질 수 있는 시스템 지도로 만듭니다. 중심부의 관계는 더 조밀하고 바깥은 점차 느슨해지며, 자연스러운 흰색 판지 위의 주홍색 또는 코랄 레드 실끈이 창발 기하를 만듭니다. 동시에 바늘땀 그림자, 교차와 가림, 손글씨 태그와 실제 수작업 오차를 남깁니다.

### 이런 경우에 적합합니다

- **피사체 사이의 관계를 이해하고 싶을 때:** 동작 방향, 공간적 위치, 구조의 전환점과 환경 단서를 노드와 연결선으로 바꿉니다.
- **실제 설치물의 질감이 필요할 때:** 컴퓨터로 그린 네트워크 도식 대신 압정, 면실, 종이 태그, 바늘땀 그림자, 장력 차이와 미세한 가림을 사용합니다.
- **복잡하지만 질서 있는 화면을 원할 때:** 하나의 핵심 노드나 관계를 축으로 삼아 중심은 조밀하고 바깥은 느슨하게 구성하며 충분한 여백을 남깁니다.
- **유연한 결과물이 필요할 때:** 상하, 좌우, 디자인 전용, 여러 비율, 4종 기기 벽지와 이미지 폴더 일괄 처리를 지원합니다.

### 빠른 시작

> 이 이미지를 XXD Panel 087로 처리하고 가장 알맞은 구도와 크기를 먼저 추천해 주세요.
...
**특징:** 실제 노드 · 붉은 실끈 링크 · 창발 기하 · 손글씨 주석 · 리서치 월 질감 · 의미 있는 여백
...
| 확인할 점 | 이 스타일의 답 |
|---|---|
| **결과물** | 촉각이 느껴지고 관계가 명확하며 연구적인 분위기를 지닌 물리적 시스템 지도 포스터 |
| **대표 특징** | 압정형 노드, 주홍색 실끈, 중심의 집약, 느슨한 바깥 영역, 종이 태그와 손글씨 관찰 흔적 |
| **입력 존중 방식** | 장면을 그대로 베끼지 않고 피사체, 구조, 자세, 방향과 공간 관계에서 의미 있는 노드와 연결을 추출함 |
...
```text
피사체·윤곽·구조·자세·서사적 관계 분석 → 의미 있는 소수의 Nodes로 분해 → 실제 실끈으로 Edges / Links 구축 → 핵심 관계를 중심으로 방사·군집·삼각·다각형·영역 횡단 연결 형성 → 중심은 조밀하고 바깥은 느슨하게 구성 → 압정·종이 태그·바늘땀 그림자·가림·수작업 오차 추가 → 노드 주변과 여백에 손글씨 주석 배치
```

## 완성작의 특징

- 모든 노드는 원본 사진의 윤곽, 구조의 전환점, 동작 방향, 위치 또는 환경 단서에 대응하며 의미 없는 노드를 추가하지 않습니다.
- 실끈은 절제되면서도 선명한 주홍색, 코랄 레드 또는 따뜻한 빨간색을 사용하고, 배경은 자연스러운 흰색, 따뜻한 흰색, 옅은 회색 또는 조화를 이루는 매우 옅은 색으로 유지합니다.
- 노드는 방사형, 군집형, 삼각형, 다각형 및 영역을 가로지르는 연결을 이룰 수 있지만, 무작위로 얽거나 화면 전체를 채워서는 안 됩니다.
- 압정, 종이 라벨, 바늘땀 그림자, 실끈 장력 차이, 교차와 가림이 실제 설치물의 촉감을 만듭니다.
- 텍스트는 소량의 손글씨 관찰 주석으로 제한하며, 사이버 UI, 범죄 수사 보드, 만화 같은 압정, 평범한 흐름도와 정형화된 마인드맵을 피합니다.

미적 제약은 [원본 프롬프트](references/original-prompt/zh-CN.md)에만 있습니다. Skill과 실행 어댑터는 납품 변수만 처리합니다. [Skill](SKILL.md) · [영문 실행 어댑터](references/xxd-panel-087-prompt.en.md)
...
이미지 폴더를 직접 전달하면 이 병사 Skill이 같은 Panel 087의 원본 미학으로 일괄 처리합니다. 일반 래스터 형식을 기본적으로 재귀 탐색하고 수량을 먼저 알린 뒤 모드·크기·텍스트·언어를 한 번만 확인합니다. 각 이미지는 내용이나 문구가 섞이지 않도록 독립 생성하며 전체 배치는 하나의 평면 작업 디렉터리만 사용합니다. 개별 실패는 기록하고 이후 이미지를 조용히 누락하지 않습니다.
...
/xxd-panel-087 photo.jpg --mode top-bottom --size auto --text prompt --locale ko-KR

# 좌우 비교: 3:2 가로형, 글자 없음
/xxd-panel-087 photo.jpg --mode left-right --size 3:2 --text none

# 디자인 전용: 9:16 휴대폰 세로형, 한국어 문구
/xxd-panel-087 photo.jpg --mode design-only --size 9:16 --text prompt --locale ko-KR
...
/xxd-panel-087 photo.jpg --mode design-only --size 3:4 --text prompt --locale ko-KR

# 정확한 문구를 재작성·번역·제목 추가 없이 사용
/xxd-panel-087 photo.jpg --mode design-only --size 3:4 --copy "빛을 이곳에 남겨 두기" --locale ko-KR

# 2개 모드 × 3개 크기로 독립적인 완성 구도 6개 생성
/xxd-panel-087 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ko-KR

# 폴더 전체에 Panel 087 적용, 공통 설정은 한 번만 확정
/xxd-panel-087 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ko-KR
...
Claude Code 사용자는 같은 폴더를 다음 위치에 연결할 수 있습니다: `~/.claude/skills/xxd-panel-087`. 설치 후 에이전트 세션을 다시 시작하세요.
...
전체 사양: [Skill 워크플로](SKILL.md) · [원본 스타일 자료](references/original-prompt/zh-CN.md) · [영문 런타임 어댑터](references/xxd-panel-087-prompt.en.md) · [중문 런타임 어댑터](references/xxd-panel-087-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 전체 프로젝트 목록

87개 Panel은 각각 독립된 원본 프롬프트와 미적 논리를 유지합니다. 아래 표에는 001부터 087까지 빠짐없이 나열하며 현재 프로젝트는 굵게 표시합니다.
...
| **병사급** | `xxd-panel-NNN` (현재 001–087) | 각 번호가 고유한 원본 프롬프트와 미학만 실행해 장군이 배정한 하나의 작업을 완성합니다. |
