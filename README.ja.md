...
作例枠は確保済みです。検証可能な原作リンク、または 087 原文プロンプトに直接照合した追加作例が揃い次第ここで優先表示し、別番号の画像や壊れたプレースホルダーで代用しません。
...
- ノードは原写真の輪郭、構造転換、動作方向、位置、環境手掛かりに対応する。
- 糸は朱赤、珊瑚赤、暖赤を主関係色とし、自然白や淡灰の紙板と対比させる。
- ピン、紙タグ、針の影、張力差、交差遮蔽で実体装置の触感を作る。
- 中央は密に、周縁は疎にし、十分な負の空間を残す。
- 文字は少量の手書き観察注釈に限り、サイバー UI、犯罪ボード、普通のフローチャートを避ける。
