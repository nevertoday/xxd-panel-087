...
`sample-01`–`sample-04` は、検証可能なオリジナル公開作例のために確保しています。上の8点は、16:9の横長・左右構成4点と、3:4の縦長・上下構成4点です。すべて Panel 087 が自身の原文プロンプトに基づいて個別に生成したもので、別番号の作品は流用していません。作例のコピーは英語で自動生成しています。

<!-- xxd-human-intro:start -->
## このスタイルが解決すること

写真を普通のフローチャートにすると空間関係が失われ、無作為な赤い糸と全面ノードは安価な捜査ボードやデジタル UI になりがちです。

**Panel 087** は主体、輪郭、構造、姿勢、物語関係を意味のあるピン状ノードへ分解し、実物の糸で再接続します。中心は密に、周縁は疎にし、朱赤や珊瑚色の糸、紙タグ、影、交差、手仕事の誤差で触れられるシステムマップを作ります。

### 向いている用途

- 動作方向、空間位置、構造の折れ、環境手掛かりをノードとリンクで可視化したい場合
- ピン、綿糸、紙タグ、針の影、張力差を備えた実物装置感が必要な場合
- 一つの核心関係を中心に、中央が密で外側が疎な秩序ある複雑さを求める場合
- 上下・左右・デザインのみ・複数比率・壁紙・フォルダ一括処理

### すぐ試す

> この画像を XXD Panel 087 で処理し、最適な構図とサイズを先に提案してください。
...
**特徴：** 実体ノード · 赤い糸のリンク · 創発する幾何 · 手書き注釈 · リサーチウォールの質感 · 意味のある余白
...
| 確認したいこと | このスタイルの答え |
|---|---|
| **得られるもの** | 元写真の関係を可視化する、触覚的で研究性のあるシステムマップポスター |
| **識別しやすい特徴** | ピン状ノード、赤い糸、中央の集積、疎な周縁、紙タグ、手書き観察痕 |
| **入力の尊重** | 主体、構造、姿勢、方向、空間から意味のあるノードと接続だけを抽出する |
...
```text
主体・輪郭・構造・姿勢・関係を分析 → 意味のあるノードへ分解 → 実物の糸で接続 → 放射・集積・三角・多角形・跨域関係を形成 → 中央を密に周縁を疎にする → ピン・紙タグ・影・交差・手仕事の誤差を加える → 少量の手書き注釈を置く
```

## 完成作の特徴

- すべてのノードを元写真の輪郭、構造の転換点、動作方向、位置、環境の手掛かりに対応させ、意味のないノードは加えません。
- 糸には、抑制が利きながら鮮やかな朱赤、コーラルレッド、または暖色系の赤を使い、背景は自然な白、暖かい白、淡いグレー、または調和するごく淡い色に保ちます。
- ノードは放射、集積、三角形、多角形、領域をまたぐ接続を形成できますが、無作為に絡ませたり画面全体を埋めたりしてはいけません。
- 画鋲、紙ラベル、針目の影、糸の張力差、交差による遮蔽を組み合わせ、実物のインスタレーションらしい触感を作ります。
- 文字は少量の手書き観察注釈にとどめ、サイバー UI、犯罪捜査ボード、漫画的な画鋲、普通のフローチャート、型通りのマインドマップを避けます。

美的制約は[原文プロンプト](references/original-prompt/zh-CN.md)だけにあります。Skill と実行アダプターは納品変数のみを扱います。 [Skill](SKILL.md) · [英語実行アダプター](references/xxd-panel-087-prompt.en.md)
...
画像フォルダーを直接渡すと、この兵士 Skill が同じ Panel 087 の原文美学で一括処理します。一般的なラスター形式を既定で再帰検索し、件数を報告してから、モード・サイズ・文字・言語を一度だけ確認します。各画像は内容や文言を混ぜず独立生成し、バッチ全体は一つのフラットなタスクフォルダーへ保存します。個別の失敗は記録し、後続画像を黙って省略しません。
...
/xxd-panel-087 photo.jpg --mode top-bottom --size auto --text prompt --locale ja-JP
...
/xxd-panel-087 photo.jpg --mode design-only --size 3:4 --text prompt --locale ja-JP

# 正確な文言を、書き換え・翻訳・タイトル追加なしで使用
/xxd-panel-087 photo.jpg --mode design-only --size 3:4 --copy "光をここに残す" --locale ja-JP

# 2モード × 3サイズで独立した完成構図を6枚生成
/xxd-panel-087 photo.jpg --mode top-bottom,design-only --size 1:1,3:4,9:16 --text prompt --locale ja-JP

# フォルダー全体へ Panel 087 を適用し、共通設定は一度だけ解決
/xxd-panel-087 "/path/to/photos" --mode design-only --size auto,9:16 --text prompt --locale ja-JP
...
Claude Code では同じフォルダを次へリンクできます： `~/.claude/skills/xxd-panel-087`. インストール後に Agent セッションを再起動してください。
...
完全仕様: [Skill ワークフロー](SKILL.md) · [原始スタイル資料](references/original-prompt/zh-CN.md) · [英語ランタイムアダプター](references/xxd-panel-087-prompt.en.md) · [中国語ランタイムアダプター](references/xxd-panel-087-prompt.zh-CN.md)

<!-- xxd-panel-catalog:start -->
## XXD Panel 全プロジェクト一覧

87 個の Panel は、それぞれ独立した原文プロンプトと美的ロジックを保持しています。表には 001 から 087 までを欠番なく掲載し、現在のプロジェクトを太字で示します。
...
| **兵士級** | `xxd-panel-NNN`（現在 001–087） | 各番号が固有の原文プロンプトと美学だけを実行し、将軍から渡された一つの仕事を完成させます。 |
